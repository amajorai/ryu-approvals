...
export function App() {
	...
	// Which notification feed the Notifications section shows: the live inbox
	// (default) or the archive. Toggling re-fetches through useNotifications.
	...
	const notifications = useNotifications(showArchived);
	...
	return (
		<PullToRefresh className="h-full" onRefresh={async () => {
			await Promise.all([approvals.refetch(), quests.refetch(), notifications.refetch(), refetch()]);
		}}>
				...
				<section className="flex flex-col gap-3">
					<div className="flex items-center justify-between gap-3">
						...
						<div className="flex items-center gap-1">
							{!showArchived && unreadNotifications > 0 ? (
...
function NotificationAppIcon({
	...
	if (tile?.glyph) {
		return (
			<span
				...
				style={tile.background ? { backgroundColor: tile.background } : undefined}
			>
	...
	return (
		<span
			...
			className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-muted text-xs font-semibold text-muted-foreground"
		>
			{seed || "RY"}
		</span>
	);
...
function NotificationRow({
	...
	return (
		<div className={`rounded-lg bg-card ${unread && !archived ? "" : "opacity-75"}`}>
			<div className="flex items-start justify-between gap-3 p-3">
