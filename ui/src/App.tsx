...
export function App() {
	...
	const unreadNotifications = notifications.notifications.filter(
		(n) => !(n.read_at || isArchivedNotification(n))
	).length;
	...
	const hasNotifications =
		!notifications.loading && filteredNotifications.length > 0;
	...
	const refreshQuests = quests.refetch;
	...
	}, [refreshApprovals, refreshQuests, refreshNotifications, refetch]);

	return (
		<PullToRefresh className="h-full" onRefresh={refreshAll}>
				...
				{allEmpty ? (
					<Empty className="py-10">
						<EmptyHeader>
							...
							<EmptyDescription>
								Approvals, finished tasks, and proactive suggestions from Ryu
								will appear here.
							</EmptyDescription>
						</EmptyHeader>
					</Empty>
						...
						{hasNotifications ? (
							<div className="flex flex-col gap-2">
								{filteredNotifications.map((n) => (
									<NotificationRow
										archived={isArchivedNotification(n)}
										key={n.id}
										notification={n}
										notifications={notifications}
									/>
							...
							<div className="rounded-lg bg-card/50 p-4 text-center text-muted-foreground text-sm">
								{notificationFilter === "archived"
									? "Nothing archived yet — rows you archive live here."
									: notificationFilter === "unread"
										? "You’re all caught up."
										: `No ${notificationFilterLabel(notificationFilter).toLowerCase()} notifications.`}
							</div>
						)}
					</section>
				) : null}
					...
					<section className="flex flex-col gap-3">
						<div className="flex items-center justify-between">
							...
							<Badge variant="default">
								{taskSuggestions.length} to review
							</Badge>
						</div>
						<div className="flex flex-col gap-2">
			...
			<div className="flex items-start justify-between gap-3 p-3">
				<div className="min-w-0">
					<div className="flex items-center gap-2">
						...
						<Badge variant="secondary">{KIND_LABEL[request.kind]}</Badge>
					</div>
...
/**
 * The sender's icon tile for a notification row: the SENDING APP's real glyph
 * (a `data:` URL inlined by the host, so it survives the frame's
 * `img-src data: blob:` CSP) on the app's plate, falling back to a deterministic
 * monogram when the app is unknown or ships no art.
 */
function NotificationAppIcon({
	name,
	notification,
	notifications,
}: {
	...
	const tile = notification.source_app_id
		? notifications.icons[notification.source_app_id]
		: undefined;
	if (tile?.glyph) {
		return (
			<span
				aria-label={tile.name}
				className="flex size-9 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-muted"
				style={
					tile.background ? { backgroundColor: tile.background } : undefined
				}
			>
				<img alt="" className="size-5" src={tile.glyph} />
			</span>
		);
	}
	// Unknown / no-art sender: a deterministic monogram seeded from the app id so
	// the row still reads as that app rather than one repeated glyph.
	const seed = (notification.source_app_id ?? name ?? notification.id)
		.replace(/^@?ryu\//, "")
		.replace(/^@/, "")
		.replace(/[^a-z0-9]/gi, "")
		.slice(0, 2)
	...
	return (
		<span
			aria-label={tile?.name ?? name ?? "Ryu"}
			className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-muted font-medium text-muted-foreground text-xs"
		>
			{seed || "RY"}
		</span>
	);
...
// that resumes the suspended workflow run once the gate policy is met; live rows
// can be archived, archived rows restored. All mutate through the shared
// useNotifications hook, whose success refetches the notifications list so the
// row updates. The poll keeps the list live regardless.
function NotificationRow({
	...
	const appName = notification.source_app_id
		? (notifications.icons[notification.source_app_id]?.name ?? null)
		: null;
	...
	return (
		<div
			className={`rounded-lg bg-card ${unread && !archived ? "" : "opacity-75"}`}
		>
			<div className="flex items-start justify-between gap-3 p-3">
				<div className="flex min-w-0 items-start gap-3">
					...
					/>
					<Button
						className="h-auto min-w-0 flex-1 shrink flex-col items-stretch justify-start whitespace-normal break-words rounded-lg p-0 text-left"
						onClick={handleOpen}
						type="button"
						variant="ghost"
					>
				...
				<div className="flex shrink-0 items-center gap-1">
					...
					{archived ? (
						<Button
							...
							className="shrink-0"
							onClick={() => {
								notifications.unarchive(notification.id).catch(() => undefined);
							}}
							size="icon"
							variant="ghost"
						>
							<HugeiconsIcon className="size-4" icon={ArchiveRestoreIcon} />
						</Button>
					) : (
						<Button
							aria-label="Archive"
							...
							variant="ghost"
						>
							<HugeiconsIcon className="size-4" icon={Archive01Icon} />
						</Button>
					)}
				</div>
			</div>
		...
		<div className="flex items-center justify-between gap-3 rounded-lg bg-card/50 p-3">
			<div className="flex min-w-0 items-center gap-2">
				...
				<div className="min-w-0">
					...
					{request.error ? (
						<p className="truncate text-status-destructive text-xs">
