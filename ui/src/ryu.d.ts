...
export interface RyuNotifications {
	 ...
	 *  glyph (CSP-safe data URL), background }`. The host inlines the icons so the
	 *  frame can render the SENDING app's icon on each row. */
	appIcons(args: { appIds: string[] }): Promise<
		Record<string, { name: string; glyph: string; background: string | null }>
	>;
	/** POST /api/notifications/:id/archive — move a row to the archive (also marks
	 *  it read). Idempotent. */
	archive(args: { id: string }): Promise<void>;
	/** GET /api/notifications — the signed-in user's inbox rows (host resolves the
	 *  user id; the sandboxed frame has no session). `archived: true` returns the
	 *  archive instead of the live inbox. */
	list(args?: { archived?: boolean }): Promise<unknown>;
	...
	unarchive(args: { id: string }): Promise<void>;
}
