...
// renders:
//
//   - `archived` (the current view) drives which list the poll/mutations refetch —
//     the live inbox by default, the archive when the user is browsing it.
...
export function useNotifications(archived = false): UseNotificationsResult {
	...
	const refresh = useCallback(async () => {
		try {
			const list = await listNotifications({ archived });
			if (aliveRef.current) {
